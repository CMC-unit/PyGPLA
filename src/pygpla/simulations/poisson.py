...
def generate_homogeneous_poisson(
    ...
    spikes = (np.random.rand(n_unit, n_bins, n_tr) < prob).astype(int)
    return spikes
...
def generate_inhomogeneous_poisson(
    ...
    hp = generate_homogeneous_poisson(base_fr, duration_s, sf, n_tr=n_tr, n_unit=n_unit)

    with np.errstate(divide="ignore", invalid="ignore"):
        ratio = fr / r_max
    ratio = np.nan_to_num(ratio, nan=0.0, posinf=0.0, neginf=0.0)
    reject = ratio > np.random.rand(*fr.shape)
